export class Agent {
    ...
    act(state: any) {
      ...
      if (!this.qTable[k]) this.qTable[k] = [0, 0];
  
      if (Math.random() < 0.1) return Math.floor(Math.random() * 2);
  
      return this.qTable[k][0] > this.qTable[k][1] ? 0 : 1;
    }
  
    update(s: any, a: number, r: number, ns: any) {
      const k = this.key(s);
      const nk = this.key(ns);
  
      if (!this.qTable[k]) this.qTable[k] = [0, 0];
      if (!this.qTable[nk]) this.qTable[nk] = [0, 0];
